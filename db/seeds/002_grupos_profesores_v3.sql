-- =============================================================================
-- ADES — Seed 002 v3: Grupos y Profesores
-- CURPs ficticios de exactamente 18 caracteres
-- Formato CURP: 4 letras + 6 fecha(AAMMDD) + H/M + 2 estado + 3 consonantes + 2 dígitos
-- Para ficticios usamos: XEXX + nivel(2) + plantel(2) + grado + grupo + seq(4) + A1
-- =============================================================================
BEGIN;
...
-- CURP ficticio exactamente 18 chars: XEXX + 2(nivel) + 2(plantel) + 1(grado) +
--   1(grupo) + 6(000000) + H + DF + NNN + seq(2)
-- Ej: XEXPAME1A000000HDFNN01  → siempre 18
-- =============================================================================
...
-- B4. Preparatoria Metepec (1 por materia = 8)
INSERT INTO ades_personas
  (nombre, apellido_paterno, apellido_materno, curp, genero)
SELECT
  'Docente ' || mat.nombre_materia,
  ...
  RPAD(
    'XEPR' ||
    'ME' ||
    LEFT(REGEXP_REPLACE(mat.nombre_materia,'[^A-Za-z]','','g'), 4) ||
    '000000HDFNN',
    16
  ) || LPAD(
    ((ROW_NUMBER() OVER (ORDER BY mat.nombre_materia)) % 100)::TEXT,
    2, '0'
  ),
  'M'
FROM ades_materias mat
...
-- D1. Primaria — titular covers subjects except English
-- (Inglés represents another table mapping)
