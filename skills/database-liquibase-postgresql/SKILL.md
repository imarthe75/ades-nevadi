...
- **Usar siempre UUID como PK:** `id UUID NOT NULL DEFAULT gen_random_uuid()`. No usar `BIGINT GENERATED ALWAYS AS IDENTITY` como PK en tablas nuevas.
- **UUID Generation:** Usar `gen_random_uuid()` (integrado en `pgcrypto`) como default. En PG18, preferir `uuidv7()` cuando esté disponible (orden temporal, menor fragmentación de índice B-tree).
- **FKs a UUID:** Toda columna que referencia una PK UUID debe declararse como `UUID` (ej. `plantel_id UUID NOT NULL REFERENCES ades_planteles(id)`).
...
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE public.nombre_tabla
            ...
            REFERENCES public.tabla_padre(id);
    END IF;
END $$;
...
COMMENT ON COLUMN public.nombre_tabla.id IS 'Llave primaria UUID generada por gen_random_uuid().';
...
-- 3. PK es UUID (gen_random_uuid()); no usar BIGINT IDENTITY.
-- 4. FKs referencian columna id de tipo UUID en tabla padre.
...
- [ ] **El PK es UUID** (`gen_random_uuid()` o `uuidv7()`); no se usa `BIGINT GENERATED ALWAYS AS IDENTITY` como PK.
- [ ] **Las FKs referencian columnas `id UUID`** de la tabla padre; los campos FK son de tipo `UUID`.
