 ...
 * CalificacionEntity no mapea relaciones JPA — solo FKs planas
 * (estudiante_id, materia_id, periodo_evaluacion_id), no aplica @EntityGraph.
 *
 * @author ADES
 * @since 2026
 */
...
public interface CalificacionJpaRepository extends JpaRepository<CalificacionEntity, UUID> {
    ...
    List<CalificacionEntity> findByPeriodoEvaluacionId(UUID periodoId);

    List<CalificacionEntity> findByMateriaId(UUID materiaId);
}
