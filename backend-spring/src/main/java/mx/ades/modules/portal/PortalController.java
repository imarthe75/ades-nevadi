...
public class PortalController {
    ...
    private String getCicloActivo(UUID estudianteId) {
    ...
    public ResponseEntity<List<Map<String, Object>>> buscarAlumnos(
        ...
        if (cicloId != null) {
            sql.append("AND ins.ciclo_escolar_id = ? ");
            params.add(cicloId);
        }
    ...
    public ResponseEntity<Map<String, Object>> resumenAlumno(
        ...
        String cicloRefStr = (cicloId != null) ? cicloId.toString() : getCicloActivo(estudianteId);
        if (cicloRefStr == null) {
        ...
        Double promedio = jdbc.queryForObject(
                "SELECT COALESCE(ROUND(AVG(cp.calificacion_final), 2), 0.0) AS promedio " +
        ...
        List<Map<String, Object>> asist = jdbc.queryForList(
                ...
                "    WHERE estudiante_id = ? AND ciclo_escolar_id = ? " +
                ")",
        ...
        if (!asist.isEmpty()) {
            ...
            if (total > 0) {
                pctAsistencia = Math.round(1000.0 * presentes / total) / 10.0;
            }
        }
        ...
        Long tareasPendientes = jdbc.queryForObject(
                ...
                "    WHERE te.tarea_id = t.id AND te.estudiante_id = ? " +
                ")",
    ...
    public ResponseEntity<List<Map<String, Object>>> calificacionesAlumno(
        ...
        for (Map<String, Object> r : rows) {
            ...
            if (!materias.containsKey(m)) {
                Map<String, Object> matData = new HashMap<>();
                matData.put("materia", m);
                matData.put("periodos", new ArrayList<Map<String, Object>>());
                matData.put("promedio", 0.0);
                materias.put(m, matData);
            }
            ...
            ((List<Map<String, Object>>) materias.get(m).get("periodos")).add(pData);
        }
        ...
        for (Map<String, Object> v : materias.values()) {
            ...
            if (count > 0) {
                v.put("promedio", Math.round(100.0 * sum / count) / 100.0);
            }
        }

        return ResponseEntity.ok(new ArrayList<>(materias.values()));
    }
